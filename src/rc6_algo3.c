...
int main() {
        ...
        unsigned char pt1[] = {0x00000000, 0x00000000, 0x00000000, 0x00000000};
        unsigned char pt2[] = {0x41, 0x41, 0x41, 0x41};
...
//        printf("\nShould be: 36a5c38f 78f7b156 4edf29c1 1ea44898\n\n");
        ...
        unsigned int * pt_aux_i = malloc(sizeof(unsigned int) * 4);
        printf("\nTest 2: ");
        printf("\nData: ");
        for (i = 0; i < 4; i++) {
        	pt_aux_i[i] = (unsigned int) pt2[i];
        	printf(" %c", pt2[i]);
        }
        rc6_block_encrypt(pt_aux_i, ct);
        printf("\nData Encrypted: ");
        for (i = 0; i < 4; i++) printf(" %08x", ct[i]);
        unsigned int * pt = malloc(sizeof(unsigned int) * 4);
		rc6_block_decrypt(ct, pt);
		unsigned char * pt_aux_c = malloc(sizeof(unsigned int) * 4);
		printf("\nData Decrypted: ");
		for (i = 0; i < 4; i++) {
			pt_aux_c[i] = (unsigned char)pt[i];
			printf(" %c", pt[i]);
		}
		printf("\n");
		free(pt);
